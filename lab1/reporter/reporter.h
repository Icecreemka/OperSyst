...
inline void Reporter::getFromFile() {
  while (!inF->eof()) {
    ...
    if (!inF->eof()) {
      lines.push_back("Employee number " + to_string(emp.num) + ": " +
                      emp.name + " work for " + strh + " hours and get " +
                      strs);
    }
  }
}

inline vector<string> Reporter::getLines() { return lines; }